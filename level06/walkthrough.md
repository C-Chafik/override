...
There is 2 function, main(), and auth().
...
This binary is tricky, first of all it takes 2 input, a string, and an integer, both gathered by an fgets(), and a scanf.

Lets look the binary behavior:
...
*               level06           *
***********************************
-> Enter Login: AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA
***********************************
***** NEW ACCOUNT DETECTED ********
***********************************
...
It takes our login, and a serial.

In the second call i tried to overflow the first input, and the program exited without asking the second input, so it might overflowed the second input.

And when we see the IDA output, since fgets() read up to 32 byte, and our buffer might weight 28 byte, then we can overflow up to 4 byte.

It then takes both input and give it to the auth() function which is really hard to undurstand.

If the auth() function return false, then a shell is prompted, so we want that function to return false.

Lets comment everything we know about that function
...
bool auth(char *s, int a2)
{
  ...
  s[strcspn(s, "\n")] = 0; // It null terminate our string since we must give a \n to go to the second input
  v5 = strnlen(s, 32);
  if ( v5 <= 5 )
    return 1; // return true if the s size is around 5
  if ( ptrace(PTRACE_TRACEME, 0, 1, 0) == -1 ) // This condition makes this binary really hard to debug, it will return if it think it is currently under debbuging
  {
    puts("\x1B[32m.---------------------------.");
    puts("\x1B[31m| !! TAMPERING DETECTED !!  |");
    puts("\x1B[32m'---------------------------'");
  ...
  else // This is where the auth begin, but its really hard to undurstand
  {
    v4 = (s[3] ^ 0x1337) + 6221293;
    for ( i = 0; i < v5; ++i )
    {
    ...
    return a2 != v4; // a2 must be equal to v4 in order to return false
  }
}
```

Im gonna try to reproduce the behavior of the content of the else statement, in order to undurstand what to do.
...
int main(int ac, char **av)
{
    ...
    decrypt = (av[1][3] ^ 0x1337) + 6221293;
    printf("first decrypt value %d\n", decrypt);
    for ( int i = 0; i < strnlen(av[1], 32); ++i )
    {
      if ( av[1][i] <= 31 )
      {
        printf("return false");
      ...
      decrypt += (decrypt ^ (unsigned int)av[1][i]) % 0x539;
      printf("decrypt after every loop %d\n", decrypt);
    }
    printf("comparing %d with %d\n", atoi(av[2]), decrypt);
    printf("result : %d\n", atoi(av[2]) != decrypt);
    return atoi(av[2]) != decrypt;
}
```
...
```sh
➜  workspace ./a.out abcdef 12
first decrypt value 6226240
decrypt after every loop 6227377
decrypt after every loop 6228379
decrypt after every loop 6229105
decrypt after every loop 6230372
decrypt after every loop 6231562
decrypt after every loop 6232802
comparing 12 with 6232802
result : 1
➜  workspace ./a.out abcdef 6232802
first decrypt value 6226240
decrypt after every loop 6227377
decrypt after every loop 6228379
decrypt after every loop 6229105
...
decrypt after every loop 6231562
decrypt after every loop 6232802
comparing 6232802 with 6232802
result : 0
➜  workspace
```

As you can see my script printed every important step.

In order to return false i just had to take the last serial value and give it with the same input and it returned false.

Lets see if its working on the binary ?
...
And its working perfectly, nice !