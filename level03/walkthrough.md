...
3 Interessting function here, main, test and decrypt
```
(gdb) info func
...
The binary is kinda big, lets see what IDA tell us about it.
...
Ok so this one is really cool, but really tricky to undurstand.


First of all the program init time() and srand() in order to generate a random number later.
It then takes our input using scanf() to withdraw an integer.
It then takes out input and execute the test() function, with as first parameter our integer input, and the value 322424845.

The test function will execute the following :

```c
// a2 is the value 322424845, and a1 is our integer input
// So if for example i gave 10, it will execute the switch with 322424845 - 10 which is 322424835
...
The switch is executing the decrypt() function with the result of (322424845 - UserIntegerInput) if its between 1 and 21, either way its executing the decrypt() function with a random number. 

And so, what does the decypt() function do ?
...
int __cdecl decrypt(char a1)
{
  ...
  *(_DWORD *)&v4[17] = __readgsdword(0x14u);
  strcpy(v4, "Q}|u`sfg~sf{}|a3"); // Inserting the string Q}|u`sfg~sf{}|a3 into v4
  v3 = strlen(v4);
  for ( i = 0; i < v3; ++i )
    v4[i] ^= a1; // Applaying the bitwise operator OR in every index of the string with the integer in parameter
  if ( !strcmp(v4, "Congratulations!") )
    return ((int (__cdecl *)(const char *))system)("/bin/sh");
  else
    return puts("\nInvalid Password");
...
The rest of the code speak for itself, if the result of the bitwise OR on every index of the string transform the string "Q}|u`sfg~sf{}|a3" into "Congratulations!" then a shell is prompted to us.

So the first goal is to find the exact value that will transform "Q}|u`sfg~sf{}|a3" to "Congratulations!".

Im going to write a C program to find that value, based on that binary program flow.
...
int main(void)
{
        ...
        timed = time(0);
        srand(timed);
        while (translate < 10000) // Will try to execute OR BITWISE with every value between 0 - 10000
        {
                decrypt = strdup("Q}|u`sfg~sf{}|a3");
                len = strlen(decrypt);
                for ( int i = 0; i < len; ++i )
                        decrypt[i] ^= translate;
...
(My script dont leak btw)

And when i execute it.

```sh
➜  workspace gcc OR.c -o decrypt
➜  workspace ./decrypt
Congratulations!
18
``

So the value 18 is the right one, we now must insert it to the program.
...
Obviously it doesn't work, since the program will do 322424845 - 18 and our value will be equal to 322424827.

So we must ajust the calcul so the result is 18, like that :
...
So lets give him 322424827 this time :
